% % Author: Luis Badesa - adapted by Samuel Tharp (ERCOT-style aggregation, RELAXED PRICING, monotone Td)
...
alpha_wind = 0.10;        % wind availability as fraction of demand
...
% --- Monotone FR delivery times (must align with cluster ordering) ---
% Cluster order (monotone Td): [Battery/Inverter, Peaker, Steam/Coal, CCGT]
InputData.Td = [2; 7; 10; 10];   % seconds

% --- FIX the nadir interval for pricing (set this from Relaxed results) ---
% Pick ONE interval index i in 1..length(InputData.Td) where you enforce nadir in pricing
InputData.nadir_interval = 1;

% =========================
%  Generator cluster design
% =========================
% ORIGINAL (conceptual) clusters were:
%  Steam/Coal (20 units), CCGT (60), Peaker (40), Battery (200)
% Reordered here to match Td monotone order: [Battery, Peaker, Steam, CCGT]

InputData.num_gen = [200; 40; 20; 60];

% No-load / commitment-like cost per online unit ($), reordered
NLHR = [0; 50; 800; 200];

% Energy marginal costs ($/MWh), reordered
HRS  = [999; 120; 35; 45];

% Small bid/penalty for providing FR ($/MW), reordered
FR_bids = [0.001; 0.001; 0.001; 0.001];

% Generation limits per unit: [Pmin, Pmax] MW, reordered
InputData.Gen_limits = [  0   10;   % Battery
                          0  200;   % Peaker
                        300  800;   % Steam/Coal
                        100  600];  % CCGT

% Inertia constants (s), reordered
InputData.H_const = [0.0; 2.0; 6.0; 4.0];

% FR capability parameters, reordered
MaxFRpercentage = [1.00; 0.90; 0.50; 0.70];
TapperSlope     = [1.00; 1.00; 0.80; 0.90];
...
y  = sdpvar(num_Clusters,1);   % RELAXED commitment (continuous number of units online)
...
% Nadir constraints (PRICING: should enforce single fixed interval using InputData.nadir_interval in setNadir)
