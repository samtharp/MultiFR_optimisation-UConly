% % Author: Luis Badesa - adapted by Samuel Tharp (ERCOT-style aggregation, monotone Td)
...
D = 24e3;                 % MW demand (single int snap shot)
...
InputData.H_const_Wind = 0;          %

% ERCOT freq +contingency assums
InputData.f_0       = 60;    % Hz ERCOT
InputData.PLossMax  = 2750;  % MW (greatest loss in ercot)
InputData.nadir_req = 0.8;   % Hz (as used in Badesa)
InputData.Rocof_max = 1;     % Hz/s

% Frequency Response delivery times
% Monotone  Td, 
% [Battery/Inverter, Peaker, CCGT, Steam/Coal]
InputData.Td = [2; 7; 10; 10]; % delays i replaced

%  Generator cluster design
% Cluster order so it is mono tone td
%  1 Battery  non synch
%  2   SCGT or peaker synch
%  3 CCGT synch
%  4  Coal synch

InputData.num_gen = [200; 40; 60; 20];   % how many units per cluster, this aggregate

% No-load / uc cost
NLHR = [0; 50; 200; 800];                % $ cost
...
InputData.Gen_limits = [ ...
    0    10;    % 1) Battery
    0   200;    % 2) Peaker
  100   600;    % 3) CCGT
  300   800];   % 4) Steam/Coal

% Inertia constants per  cluster
InputData.H_const = [0.0; 2.0; 4.0; 6.0];

% FR capability parameters (dimension = num_Clusters x 1)
MaxFRpercentage = [1.00; 0.90; 0.70; 0.50]; % fraction of Pmax available as FR capacity per unit
TapperSlope     = [1.00; 1.00; 0.90; 0.80]; % tapering of FR vs headroom
...
max_deloading = 0;             % MW keeping same

%% Create optimisation problem
num_Clusters = length(NLHR);

x  = sdpvar(num_Clusters,1);   % MW produced by each cluster
y  = intvar(num_Clusters,1);   % integer commitment (number of units online in each cluster)
...
% FR limits
FR_limits = [ ...
    0 <= FR <= y .* InputData.FR_capacity, ...                           % only online units provide FR
    FR <= TapperSlope .* (y .* InputData.Gen_limits(:,2) - x) ];         % FR limited by headroom (tapering)
